...
try{
 ...
 const count=Number((await page.locator('#position').textContent()).split('/')[1]);assert.equal(count,20);
 ...
 assert.match(await page.locator('#position').textContent(),/^1 \/ 20$/);
 await page.clock.fastForward(60000);assert.match(await page.locator('#position').textContent(),/^1 \/ 20$/);
 await page.keyboard.press('Space');await page.clock.fastForward(19000);assert.match(await page.locator('#position').textContent(),/^2 \/ 20$/);
 await page.keyboard.press('Space');await page.clock.setSystemTime(new Date('2026-09-18T07:00:00Z'));await page.clock.fastForward(1000);assert.equal(await page.locator('h1').textContent(),'Tea Time');assert.match(await page.locator('#position').textContent(),/\/ 18$/);
 ...
 await page.reload();await page.waitForSelector('.event-title');assert.match(await page.locator('#weather-note').textContent(),/Last known/);
 await page.evaluate(()=>localStorage.removeItem('mcs-weather'));await page.reload();await page.waitForSelector('.event-title');assert.match(await page.locator('#weather').textContent(),/unavailable/);
 ...
 console.log('PASS: 20 views; official fonts; 1080p overflow and images; reduced previews; titles; pause/wrap/timer; event cutoff while paused; stale/unavailable weather; empty categories.');
