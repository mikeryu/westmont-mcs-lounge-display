...
try{
 ...
 const count=Number((await page.locator('#position').textContent()).split('/')[1]);assert.equal(count,16);
 ...
 assert.match(await page.locator('#position').textContent(),/^1 \/ 16$/);
 await page.clock.fastForward(60000);assert.match(await page.locator('#position').textContent(),/^1 \/ 16$/);
 await page.keyboard.press('Space');await page.clock.fastForward(19000);assert.match(await page.locator('#position').textContent(),/^2 \/ 16$/);
 await page.keyboard.press('Space');await page.clock.setSystemTime(new Date('2026-09-18T07:00:00Z'));await page.clock.fastForward(1000);assert.equal(await page.locator('.event-title').count(),0);assert.match(await page.locator('#position').textContent(),/\/ 15$/);
 ...
 await page.route('**/content.json',route=>route.fulfill({json:data}));await page.reload();await page.waitForSelector('.welcome-copy');assert.match(await page.locator('#position').textContent(),/1 \/ 2/);await page.keyboard.press('ArrowRight');assert.ok(await page.locator('#large-clock').textContent());
 assert.deepEqual(errors,[]);await writeFile('docs/screenshots/v2/font-check.json',JSON.stringify(fontRecords,null,2));
 console.log('PASS: 16 views; official fonts; 1080p overflow and images; reduced previews; titles; pause/wrap/timer; event cutoff while paused; stale/unavailable weather; empty categories.');
